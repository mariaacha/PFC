...
/**
 * An open addressed hashing implementation for #k#/#v# primitive entries.
 *
...
abstract public class TIntIntHash extends TPrimitiveHash {
    ...
    /**
     * Creates a new <code>T#K##V#Hash</code> instance with a prime
     ...
     *
     * @param val an <code>#k#</code> value
    ...
    private class ForEach1 implements Runnable	{
    	...
    	ForEach1 (int a, int b, TIntProcedure proc)	{
    		min = a; max = b; procedure = proc;
    	}
    	public void run ()	{
            for (int i=min; i<max; i++) {
                if (_states[i] == FULL && ! procedure.execute(_set[i] ) ) {
                    boolGlobal = false;
                }
            }
    	}
    ...
    public boolean forEach (TIntProcedure procedure)	{
    	...
    	for (int i=0; i<numProcs; i++)	{
    		...
    		new Thread (new ForEach1 (i*numElemProc, numMax, procedure)).start();
    	}
    	return boolGlobal;
    }
    /**public boolean forEach( TIntProcedure procedure ) {
     ...
     */
         ...
         protected int insertKey (int val ) {
             ...
             // already FULL or REMOVED, must probe
             return insertKeyRehash2(val, index, hash, state);
         }

         int insertKeyRehash(int val, int index, int hash, byte state) {
             // compute the double hash
        	 System.out.println("insertKeyRehash");
             ...
             do {
                 ...
                 synchronized (objectPut)	{
                	 if (state == REMOVED && (!this.contains(val)))	{
                		 insertKeyAt (index, val);
                		 return index;
                	 }
                 }
...
/**                	 if (state == FREE) {
         ...
         int insertKeyRehash2(int val, int index, int hash, byte state) {
             // compute the double hash
             final int length = _set.length;
             int probe = 1 + (hash % (length - 2));
             final int loopIndex = index;
